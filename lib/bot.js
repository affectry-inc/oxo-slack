...
function ZangyoBot(controller) {
  ...
  function getGameBoard(message, cells_j, page) {
    var reply = {
      'text': '<@' + message.user + '> gets cell-1.',
      'attachments': [{
        'title': 'Now it\'s your turn. You are \'O\'.',
        'text': getCells(cells_j),
        'fallback': 'brhaaa',
        ...
        'actions': getActions(cells_j, page)
      }]
    };
    return reply;
  };
  ...
  zangyo_bot.startGame = function(bot, message) {
    // bot.reply(message, getGameBoard(message, {'3': 'X', '5': 'X', '7': 'O', '8': 'O', '9': 'X'}, 1));
    bot.reply(message, getGameBoard(message, {}, 1));
  };
