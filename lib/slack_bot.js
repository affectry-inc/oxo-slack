...
function SlackBot(controller) {
  ...
  slack_bot.leaderboard = function(bot, message) {
    ...
    controller.storage.summaries.select({team_id: team_id}, {score: 1, win: 1}, function(err, summaries) {
      if (err) {
        console.log(err);
      } else {
        if (summaries.length == 0) {
          var reply = 'There is no record in your team yet. :ghost:'
        } else {
          var fields = [];
          for (var i = 0; i < summaries.length; i++) {
            var summary = summaries[i];
            var field = {
              'text': (i+1) + '. [' + summary.score + '] <@' + summary.user + '>',
              'short': false
            }
            fields.push(field);
          }
