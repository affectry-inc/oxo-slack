...
function SlackBot(controller) {
  ...
  function findWinner(game) {
    if (Object.keys(game.cells).length >= 5){
      ...
      var cnt_draw = 0, is_x_reached = false;
      for (var i = 0; i < 8; i++) {
        var line = lines[i];
        var cnt_o = 0, cnt_x = 0;
        for (var j = 0; j < 3; j++) {
          console.log('i,j:'+i+','+j+' cell:'+ line[j] + ' value:' + game.cells[line[j]]);
      ...
      if ((cnt_draw === 7 && is_x_reached) || cnt_draw === 8) return 'DRAW';
    }
    return null;
  };
